...
st.set_page_config(page_title="predicción de compra de videojuengos", layout="centered")
...
#Creamos el sidebar
st.sidebar.title("informacion del ususarios")
...
def main():
    ...
    def user_input_feature():
        edad = st.sidebar.number_input("Edad", 10, 30)

        option_juego = ['black', 'god hand', 'Dead Space', 'god of war', 'Fifa', 'F1', 'KOA: Reckoning']
        videojuego = st.sidebar.selectbox('Seleccione el tipo de videojuego', option_juego)

        option_plataforma = ['Play Station', 'nintendo', 'Xbox', 'Otros']
        plataforma = st.sidebar.selectbox('Plataforma', option_plataforma)
    ...
    st.subheader("Datos que ingresan modelo")
    data_preparada
